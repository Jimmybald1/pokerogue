...
export abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  private generateAbilityIndex(): number {
    ...
    const regularAbility = this.species.ability2 === this.species.ability1 ? 0 : randSeedInt(2, undefined, "Random normal ability");
    
    LoggerTools.TestAllAbilityCharms(BASE_HIDDEN_ABILITY_RATE, this.species.abilityHidden);
  ...
  public tryRerollHiddenAbilitySeed(haThreshold: number): void {
    ...
    if (!randSeedInt(hiddenAbilityChance.value, undefined, "Hidden Ability chance")) {
      this.abilityIndex = 2;
    }
  }
  ...
  canBeCrit(simulated: boolean = false): boolean {
    ...
    if (noCritTag || blockCrit.value || activeOverrides.CRITICAL_HIT_OVERRIDE) {
      return false;
    }
    return true;
  }
  ...
  getAttackDamage({
    ...
    damage.value = simulated ? calculatedDamage : toDmgValue(calculatedDamage); // Pathing Tool force simulated check to ignore random damage deviation
  ...
  getCriticalHitResult(source: Pokemon, move: Move): boolean {
    ...
    isCritical ||= activeOverrides.CRITICAL_HIT_OVERRIDE ?? globalScene.randBattleSeedInt(critChance, undefined, "Crit chance (1 in " + critChance + ")") === 0;
  ...
  public doSetStatus(
    effect: StatusEffect,
    sleepTurnsRemaining = effect === StatusEffect.SLEEP ? (this.randBattleSeedInt(3, undefined, "Random sleep turns") === 0 ? 2 : 3) : 0,
  ): void {
  ...
  randBattleSeedIntRange(min: number, max: number, reason?: string): number {
    return this.randBattleSeedInt(max - min + 1, min, reason);
  }
...
export class EnemyPokemon extends Pokemon {
      ...
      switch (this.aiType) {
        ...
        case AiType.SMART: {
          ...
          if (this.aiType === AiType.SMART_RANDOM) {
            ...
            while (chosenMoveIndex < sortedMovePool.length - 1 && globalScene.randBattleSeedInt(8, undefined, "Smart-Random AI Move Selection") >= 5) {
              chosenMoveIndex++;
            }
  ...
  private handleBossSegmentCleared(segmentIndex: number): void {
    ...
    while (this.bossSegmentIndex > 0 && segmentIndex - 1 < this.bossSegmentIndex) {
      ...
      const boostedStat = weightedPick(new Map(leftoverStats.map(s => [s, this.getStat(s, false)])), "Boss Segment Random Stat Boost");
  ...
  calculateBossShieldRequirements(): integer[] {
    ...
    if (this.isBoss()) {
      ...
      if (this.hp >= roundedHpThreshold) {
        ...
        while (segmentsBypassed < this.bossSegmentIndex) {
