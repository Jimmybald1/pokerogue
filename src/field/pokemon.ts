...
import { NumberHolder, randSeedInt, getIvsFromId, BooleanHolder, randSeedItem, isNullOrUndefined, getEnumValues, toDmgValue, fixedInt, rgbaToInt, rgbHexToRgba, rgbToHsv, deltaRgb, isBetween, type nil, type Constructor, getEnumKeys } from "#app/utils";
...
export default abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    if (abilityIndex !== undefined) {
      ...
      const randAbilityIndex = randSeedInt(2, undefined, "Selecting ability index");
...
   */
  ...
  apply(source: Pokemon, move: Move): HitResult {
    ...
    if (critOnly.value || critAlways) {
      ...
      isCritical =
        critChance === 1 || !globalScene.randBattleSeedInt(critChance, undefined, "Crit chance (1 in " + critChance + ")");
    }
  ...
  trySetStatus(
    ...
    if (effect === StatusEffect.SLEEP) {
      sleepTurnsRemaining = new NumberHolder(this.randSeedIntRange(2, 4, "Random sleep turns"));
   ...
   */
  randSeedInt(range: number, min = 0, reason?: string): number {
