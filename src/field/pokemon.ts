...
export default abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  getAttackDamage(
    ...
    isCritical = false,
    simulated = true,
    config: 0 | 1 | 2 = 0): DamageCalculationResult {
    ...
    applyMoveAttrs(
      ...
      this,
      move,
      variableCategory, config && config > 0 ? "SIM" : undefined,
    );
    ...
    const randomMultiplier = simulated
      ? 1
      : (config === 0 ? this.randSeedIntRange(85, 100, "Attack damage") / 100 : (config === 1 ? 0.85 : 1));
