...
import { NumberHolder, randSeedInt, getIvsFromId, BooleanHolder, randSeedItem, isNullOrUndefined, getEnumValues, toDmgValue, fixedInt, rgbaToInt, rgbHexToRgba, rgbToHsv, deltaRgb, isBetween, type nil, type Constructor, getEnumKeys, randSeedIntRange } from "#app/utils/common";
...
   */
  ...
  public generateAndPopulateMoveset(): void {
    ...
    if (this.hasTrainer() || this.isBoss()) {
      ...
      if (attackMovePool.length) {
        ...
        let rand = randSeedInt(totalWeight, undefined, "Selecting a damage dealing move to include");
        let index = 0;
  ...
  getAttackDamage(
    ...
    applyMoveAttrs(
      ...
      this,
      move,
      variableCategory, simulated ? "SIM" : undefined,
    );
  ...
  */
  getCriticalHitResult(source: Pokemon, move: Move, simulated: boolean = true): boolean {
    ...
    if (!isCritical.value) {
      ...
      isCritical.value = critChance === 1 || !globalScene.randBattleSeedInt(critChance, undefined, "Crit chance (1 in " + critChance + ")");
    }
  ...
  */
  ...
  randBattleSeedIntRange(min: number, max: number, reason?: string): number {
    return globalScene.currentBattle
      ? globalScene.randBattleSeedInt(max - min + 1, min, reason)
...
export class EnemyPokemon extends Pokemon {
        ...
        case AiType.SMART:
          ...
          if (this.aiType === AiType.SMART_RANDOM) {
            ...
            while (
              ...
              globalScene.randBattleSeedInt(8, undefined, "Smart-Random AI Move Selection") >= 5
            ) {
              r++;
            }
  ...
  getNextTargets(moveId: Moves): BattlerIndex[] {
    if (doMoveLogging) {
      console.log("Starting getNextTargets() for " + this.name + " with move " + getEnumKeys(Moves)[moveId]);
    }
