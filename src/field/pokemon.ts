...
export default abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    if (dataSource) {
      ...
      this.id = randSeedInt(4294967296, undefined, "Generating a Pokemon ID to create Pokemon's IVs");
  ...
  private generateAbilityIndex(): number {
    if (this.species.abilityHidden) {
      ...
      globalScene.executeWithoutSeedAdvancement(() => {
        haRolls.push(randSeedInt(BASE_HIDDEN_ABILITY_CHANCE, undefined, "Hidden Ability chance 0 charms"));
      });
      
      globalScene.executeWithoutSeedAdvancement(() => {
        haRolls.push(randSeedInt(BASE_HIDDEN_ABILITY_CHANCE/4, undefined, "Hidden Ability chance 1 charm")); // first charm is 4x
      });
      
      globalScene.executeWithoutSeedAdvancement(() => {
        haRolls.push(randSeedInt(BASE_HIDDEN_ABILITY_CHANCE/8, undefined, "Hidden Ability chance 2 charms"));
      });
      
      globalScene.executeWithoutSeedAdvancement(() => {
        haRolls.push(randSeedInt(BASE_HIDDEN_ABILITY_CHANCE/16, undefined, "Hidden Ability chance 3 charms"));
      });
      
      globalScene.executeWithoutSeedAdvancement(() => {
        haRolls.push(randSeedInt(BASE_HIDDEN_ABILITY_CHANCE/32, undefined, "Hidden Ability chance 4 charms"));
      });
  ...
  generateNature(naturePool?: Nature[]): void {
    ...
    const nature = naturePool[randSeedInt(naturePool.length, undefined, "Random nature")];
    this.setNature(nature);
  }
  ...
  protected generateShinyVariant(): Variant {
    ...
    globalScene.executeWithSeedOffset(
      () => {
        rand.value = randSeedInt(10, undefined, "Random variant selection");
      },
      this.id,
  ...
  public generateFusionSpecies(forStarter?: boolean): void {
    ...
    const hasHiddenAbility = !randSeedInt(hiddenAbilityChance.value, undefined, "Whether the Pokemon has its HA or not");
    const randAbilityIndex = randSeedInt(2, undefined, "Ability slot (if no HA)");
  ...
  public generateAndPopulateMoveset(): void {
    ...
    if (stabMovePool.length) {
      ...
      let rand = randSeedInt(totalWeight, undefined, "Selecting a STAB move to include");
      let index = 0;
    ...
    while (baseWeights.length > this.moveset.length && this.moveset.length < 4) {
      ...
      let rand = randSeedInt(totalWeight, undefined, "Selecting moves");
      let index = 0;
   ...
   */
  canBeCrit(simulated: boolean = false): boolean {
    ...
    applyAbAttrs("BlockCritAbAttr", { pokemon: this, passive: undefined, blockCrit: blockCrit, simulated: simulated });
    if (noCritTag || blockCrit.value || Overrides.CRITICAL_HIT_OVERRIDE) {
      return false;
    }
    return true;
  }
  ...
  isGuaranteedCrit(source: Pokemon, move: Move, simulated: boolean = false): boolean {
    ...
    applyAbAttrs("ConditionalCritAbAttr", { pokemon: source, move: move, target: this, passive: undefined, isCritical: critOnly, simulated: simulated });
  ...
  getAttackDamage({
     ...
     */
    const randomMultiplier = simulated ? 1 : this.randBattleSeedIntRange(85, 100, "Attack damage") / 100;
    ...
    damage.value = simulated ? calculatedDamage : toDmgValue(calculatedDamage);
  ...
  getCriticalHitResult(source: Pokemon, move: Move): boolean {
    ...
    isCritical ||= Overrides.CRITICAL_HIT_OVERRIDE ?? globalScene.randBattleSeedInt(critChance, undefined, "Crit chance (1 in " + critChance + ")") === 0;
  ...
  trySetStatus(
    ...
    if (effect === StatusEffect.SLEEP) {
      sleepTurnsRemaining = new NumberHolder(this.randBattleSeedIntRange(2, 4, "Random sleep turns"));
  ...
  updateFusionPalette(ignoreOveride?: boolean): void {
    ...
    Math.random = () => randSeedFloat("Update fusion palette");
   ...
   */
  randBattleSeedInt(range: number, min = 0, reason?: string): number {
    return globalScene.currentBattle ? globalScene.randBattleSeedInt(range, min, reason) : randSeedInt(range, min, reason);
  }
...
export class EnemyPokemon extends Pokemon {
  ...
  getNextMove(): TurnMove {
    ...
    if (movePool.length) {
      ...
      switch (this.aiType) {
        ...
        case AiType.RANDOM: {
          const moveId = movePool[globalScene.randBattleSeedInt(movePool.length, undefined, "Move selection roll (RANDOM)")].moveId;
          ...
          if (this.aiType === AiType.SMART_RANDOM) {
            ...
            while (
              ...
              moveScores[movePool.indexOf(sortedMovePool[r + 1])] / moveScores[movePool.indexOf(sortedMovePool[r])] >=
                0 &&
              globalScene.randBattleSeedInt(100, undefined, "Smart AI Move Selection") <
                Math.round(
  ...
  handleBossSegmentCleared(segmentIndex: number): void {
    while (this.bossSegmentIndex > 0 && segmentIndex - 1 < this.bossSegmentIndex) {
      ...
      const randInt = randSeedInt(totalWeight, undefined, "Random stat to raise from breaking a segment");
  ...
  setBattleInfoFlyout(natureName: string, ivDesc: string, abilityName: string, passiveName: string, abilityIndex: number): void {
    this.battleInfo.setBattleInfoFlyout(natureName, ivDesc, abilityName, passiveName, abilityIndex);
  }
}
