...
export class Arena {
  ...
  public pokemonPool: ArenaPokemonPools;
  ...
  public randomTrainerType(waveIndex: number, isBoss = false): TrainerType {
    ...
    return tierPool.length > 0 ? randSeedItem(tierPool, "Selecting trainer type") : TrainerType.BREEDER;
  }
  ...
  public randomSpecies(waveIndex: number, level: number, attempt = 0, luckValue = 0, isBoss?: boolean): PokemonSpecies {
    ...
    if (forcedTier === null) {
      ...
      const rngRoll = randSeedInt(rollMax - luckModifier, undefined, "Selecting rarity tier for encounter");
      tier = (isBossSpecies ? this.generateBossBiomeTier : this.generateNonBossBiomeTier)(rngRoll);
    } else {
      tier = forcedTier;
    }

    if (LoggerTools.logRNG) console.log("Starting species pool tier:", BiomePoolTier[tier]);
    ...
    if (tierPool.length === 0) {
      ...
      species = speciesDataRegistry.getSpecies(randSeedItem(tierPool, "Selecting rarity tier but for real this time"));
    }
    ...
    if (newSpeciesId !== species.speciesId) {
      if (LoggerTools.logRNG) console.log("Replaced", SpeciesId[species.speciesId], "with", SpeciesId[newSpeciesId]);
