...
export class Trainer extends Phaser.GameObjects.Container {
  ...
  constructor(
    ...
    this.partyTemplateIndex = Math.min(
      partyTemplateIndex !== undefined ? partyTemplateIndex : randSeedItem(this.config.partyTemplates.map((_, i) => i), "Random trainer party template"),
    ...
    if (i18next.exists(classKey, { returnObjects: true })) {
      if (nameKey) {
        ...
        const genderKey = i18next.exists(`${classKey}.male`)
          ? variant === TrainerVariant.FEMALE
            ...
            : ".male"
          : "";
        [this.nameKey, this.name] = getRandomLocaleEntry(`${classKey}${genderKey}`, "Trainer name 1");
      }
      ...
      if (variant === TrainerVariant.DOUBLE) {
        if (this.config.doubleOnly) {
          ...
          [this.partnerNameKey, this.partnerName] = getRandomLocaleEntry(`${classKey}${partnerGenderKey}`, "Trainer name 2");
        }
      }
    }
  ...
  genNewPartyMemberSpecies(level: number, strength: PartyMemberStrength, attempt?: number): PokemonSpecies {
    ...
    if (this.config.speciesPools) {
      ...
      let rolledSpecies = randSeedItem(tierPool, "Random party member species");
    ...
    if (LoggerTools.logRNG) console.log(ret.getName());
    ...
    if (this.checkDuplicateSpecies(baseSpecies.speciesId)) {
      if (LoggerTools.logRNG) console.log("Duplicate species detected, prompting reroll...");
      retry = true;
    }

    if (retry && (attempt ?? 0) < 10) {
      if (LoggerTools.logRNG) console.log("Rerolling party member...");
