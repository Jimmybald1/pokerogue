...
export default class Battle {
  ...
  randSeedGaussForLevel(value: number): number {
    ...
    for (let i = value; i > 0; i--) {
      rand += randSeedFloat("Level offset");
    }
    return rand / value;
  }
  ...
  return () => {
    ...
    globalScene.executeWithSeedOffset(() => {
      for (const trainerPoolEntry of trainerPool) {
        const trainerType = Array.isArray(trainerPoolEntry) ? randSeedItem(trainerPoolEntry, "Random trainer helper function") : trainerPoolEntry;
    ...
    if (randomGender) {
      trainerGender = randInt(2, undefined, "Random trainer helper function") === 0 ? TrainerVariant.FEMALE : TrainerVariant.DEFAULT;
    }
    ...
    if (trainerConfigs[trainerTypes[rand]].hasDouble && isEvilTeamGrunt) {
      return new Trainer(trainerTypes[rand], randInt(3, undefined, "Evil grunt selection") === 0 ? TrainerVariant.DOUBLE : trainerGender);
    }
...
export const classicFixedBattles: FixedBattleConfigs = {
  [ClassicFixedBossWaves.TOWN_YOUNGSTER]: new FixedBattleConfig()
    ...
    .setGetTrainerFunc(
      () => new Trainer(TrainerType.YOUNGSTER, randSeedInt(2, undefined, "Youngster gender") ? TrainerVariant.FEMALE : TrainerVariant.DEFAULT),
    ),
