...
export class GameMode implements GameModeConfig {
  ...
  isWaveTrainer(waveIndex: number, arena: Arena): boolean {
    ...
    if (waveIndex % 10 !== 1 && waveIndex % 10) {
        ...
        for (let w = Math.max(waveIndex - 3, waveBase + 2); w <= Math.min(waveIndex + 3, waveBase + 9); w++) {
          ...
          if (w < waveIndex) {
            globalScene.executeWithSeedOffset(() => {
              ...
              if (!randSeedInt(waveTrainerChance, undefined, "Random chance of wave being a Trainer Battle")) {
                allowTrainerBattle = false;
              }
            }, w);
      ...
      return Boolean(allowTrainerBattle && trainerChance && !randSeedInt(trainerChance, undefined, "Random chance of wave being a Trainer Battle"));
    }
    return false;
  }
  ...
  getOverrideSpecies(waveIndex: number): PokemonSpecies | null {
    if (this.isDaily && this.isWaveFinal(waveIndex)) {
      ...
      return randSeedItem(allFinalBossSpecies, "Final Boss override");
    }
