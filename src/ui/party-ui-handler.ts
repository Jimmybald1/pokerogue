...
import { BerryModifier, type PokemonFormChangeItemModifier, type PokemonHeldItemModifier } from "#modifiers/modifier";
...
export class PartyUiHandler extends MessageUiHandler {
  ...
  private processActionButtonForOptions(option: PartyOption) {
    ...
    if (
      ...
      this.clearOptions();
      LoggerTools.logCommand.value = false;
  ...
  getReleaseMessage(pokemonName: string): string {
    const rand = randInt(128, undefined, "Random release message");
    if (rand < 20) {
