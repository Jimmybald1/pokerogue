  ...
  updateIvs(ivs: number[], originalIvs?: number[], max?: integer): void {
    if (ivs) {
      ...
      const ivChartData = new Array(6)
        .fill(null)
        .flatMap((_, i) => [
          (ivs[ivChartStatIndexes[i]] / maxIV) * ivChartSize * ivChartStatCoordMultipliers[ivChartStatIndexes[i]][0],
          (ivs[ivChartStatIndexes[i]] / maxIV) * ivChartSize * ivChartStatCoordMultipliers[ivChartStatIndexes[i]][1],
        ]);
