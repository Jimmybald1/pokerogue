...
export class BypassSpeedChanceModifier extends PokemonHeldItemModifier {
  ...
  override apply(pokemon: Pokemon): boolean {
    if (pokemon.randBattleSeedInt(10, undefined, "Chance to activate Quick Claw") < this.getStackCount() && pokemon.addTag(BattlerTagType.BYPASS_SPEED)) {
...
export class ContactHeldItemTransferChanceModifier extends HeldItemTransferModifier {
  ...
  getTransferredItemCount(simulated?: "PASS" | "FAIL"): number {
    if (simulated == "PASS") {
      return 1;
    }
    if (simulated == "FAIL") {
    ...
    return randSeedFloat("Transferred item chance") <= this.chance * this.getStackCount() ? 1 : 0;
  }
...
export class EnemyAttackStatusEffectChanceModifier extends EnemyPersistentModifier {
   ...
   */
  override apply(enemyPokemon: Pokemon): boolean {
    if (randSeedFloat("Enemy attack status effect chance") <= this.chance * this.getStackCount()) {
...
export class EnemyStatusEffectHealChanceModifier extends EnemyPersistentModifier {
   ...
   */
  override apply(enemyPokemon: Pokemon): boolean {
    if (!enemyPokemon.status || randSeedFloat("Enemy status effect heal chance") > this.chance * this.getStackCount()) {
      return false;
    }
...
export class EnemyFusionChanceModifier extends EnemyPersistentModifier {
  ...
  override apply(isFusion: BooleanHolder): boolean {
    if (randSeedFloat("Enemy fusion chance") > this.chance * this.getStackCount()) {
      return false;
    }
