...
export class Egg {
  ...
  public generatePlayerPokemon(): PlayerPokemon {
    ...
    const generatePlayerPokemonHelper = () => {
      ...
      if (this._species === Species.PHIONE && this._sourceType === EggSourceType.SAME_SPECIES_EGG) {
        pokemonSpecies = getPokemonSpecies(randSeedInt(MANAPHY_EGG_MANAPHY_RATE, undefined, "Chance of Manaphy Egg not scamming you") ? Species.PHIONE : Species.MANAPHY);
      }
  ...
  private rollEggMoveIndex() {
    ...
    return randSeedInt(baseChance * tierMultiplier, undefined, "Choosing whether to give Rare Egg Move or not") ? randSeedInt(3, undefined, "Common Egg Move selection") : 3;
  }
  ...
  private rollSpecies(): Species | null {
    ...
    if (this.tier === EggTier.LEGENDARY && this._sourceType === EggSourceType.GACHA_LEGENDARY) {
      if (!randSeedInt(2, undefined, "Chance to replace Legendary Egg with gacha target")) {
