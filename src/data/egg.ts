...
export class Egg {
  ...
  constructor(eggOptions?: IEggOptions) {
    const generateEggProperties = (eggOptions?: IEggOptions) => {
      ...
      this._id = eggOptions?.id ?? randInt(EGG_SEED, EGG_SEED * this._tier, "eg");
    ...
    const generatePlayerPokemonHelper = () => {
      ...
      if (this._species === SpeciesId.PHIONE && this._sourceType === EggSourceType.SAME_SPECIES_EGG) {
        pokemonSpecies = getPokemonSpecies(
          randSeedInt(MANAPHY_EGG_MANAPHY_RATE, undefined, "Chance of Manaphy Egg not scamming you") ? SpeciesId.PHIONE : SpeciesId.MANAPHY,
        );
      }
      ...
      const sameSpeciesEggHACheck =
        this._sourceType === EggSourceType.SAME_SPECIES_EGG && !randSeedInt(SAME_SPECIES_EGG_HA_RATE, undefined, "Hidden Ability chance (Candy egg)");
      const gachaEggHACheck = !(this._sourceType === EggSourceType.SAME_SPECIES_EGG) && !randSeedInt(GACHA_EGG_HA_RATE, undefined, "Hidden Ability chance (Gacha)");
      ...
      const secondaryIvs = getIvsFromId(randSeedInt(4294967295, undefined, "Egg IVs"));
  ...
  private rollEggMoveIndex() {
    ...
    return randSeedInt(baseChance, undefined, "Choosing whether to give Rare Egg Move or not") ? randSeedInt(3, undefined, "Common Egg Move selection") : 3;
  }
  ...
  private rollEggTier(): EggTier {
    ...
    const tierValue = randInt(256, undefined, "Choosing egg tier");
  ...
  private rollSpecies(): SpeciesId | null {
    ...
    if (this.isManaphyEgg()) {
       ...
       */
      const rand = randSeedInt(MANAPHY_EGG_MANAPHY_RATE, undefined, "Manaphy Egg chance") !== 1;
      return rand ? SpeciesId.PHIONE : SpeciesId.MANAPHY;
    }
    if (this.tier === EggTier.LEGENDARY && this._sourceType === EggSourceType.GACHA_LEGENDARY && !randSeedInt(2, undefined, "Chance to replace Legendary Egg with gacha target")) {
    ...
    const rand = randSeedInt(totalWeight, undefined, "Random egg species");
  ...
  private rollShiny(): boolean {
    ...
    return !randSeedInt(shinyChance, undefined, "Shiny chance");
  }
  ...
  private rollVariant(): VariantTier {
    ...
    const rand = randSeedInt(10, undefined, "Shiny variant selection");
