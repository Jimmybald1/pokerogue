  ...
  constructor() {
    super(p => {
      ...
      if (p.moveset.filter(m => m.moveId === Moves.HYPER_DRILL).length > 0) {
        globalScene.executeWithSeedOffset(() => ret = !randSeedInt(4, undefined, "Dunsparce form selection"), p.id);
      }
      return ret;
    });
...
class TandemausEvolutionCondition extends SpeciesEvolutionCondition {
  constructor() {
    super(p => {
      let ret = false;
      globalScene.executeWithSeedOffset(() => ret = !randSeedInt(4, undefined, "Tandemaus form selection"), p.id);
      return ret;
    });
  }
}
