...
export abstract class PokemonSpeciesForm {
  ...
  generateCandyColors(): number[][] {
    ...
    Math.random = () => randSeedFloat("Generate candy colors");
...
export class PokemonSpecies extends PokemonSpeciesForm implements Localizable {
  ...
  generateGender(): Gender {
    ...
    if (randSeedFloat("Generate Gender") * 100 <= this.malePercent) {
      return Gender.MALE;
    }
  ...
  getSpeciesForLevel(
    ...
    const randValue = evolutionPool.size === 1 ? 0 : randSeedInt(totalWeight, undefined, "Random levelled species");
      ...
      for (let l = 1; l < prevolutionLevels.length; l++) {
        ...
        ret.push([
          ...
          Math.min(
            Math.max(
              evolution?.level! +
                Math.round(randSeedGauss(0.5, 1 + levelDiff * 0.2, "Prevolution level diff") * Math.max(evolution?.wildDelay!, 0.5) * 5) -
                1,
              2,
              evolution?.level!,
            ),
        ...
        Math.min(
          Math.max(
            lastPrevolutionLevel +
              Math.round(randSeedGauss(0.5, 1 + levelDiff * 0.2, "Last prevolution level diff") * Math.max(evolution?.wildDelay!, 0.5) * 5),
