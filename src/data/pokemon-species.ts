      ...
      for (let l = 1; l < prevolutionLevels.length; l++) {
        ...
        ret.push([
          ...
          Math.min(
            Math.max(
              evolution?.level! +
                Math.round(Utils.randSeedGauss(0.5, 1 + levelDiff * 0.2, "Prevolution level diff") * Math.max(evolution?.wildDelay!, 0.5) * 5) -
                1,
              2,
              evolution?.level!,
            ),
        ...
        Math.min(
          Math.max(
            lastPrevolutionLevel +
              Math.round(Utils.randSeedGauss(0.5, 1 + levelDiff * 0.2, "Last prevolution level diff") * Math.max(evolution?.wildDelay!, 0.5) * 5),
