...
export class PostDefendContactApplyStatusEffectAbAttr extends PostDefendAbAttr {
  ...
  override canApply({ pokemon, move, opponent: attacker }: PostMoveInteractionAbAttrParams): boolean {
    const effect =
      this.effects.length === 1 ? this.effects[0] : this.effects[pokemon.randBattleSeedInt(this.effects.length, undefined, "Choosing status to apply")];
    return (
...
export class PostDefendContactApplyTagChanceAbAttr extends PostDefendAbAttr {
  ...
  override canApply({ move, pokemon, opponent }: PostMoveInteractionAbAttrParams): boolean {
    return (
      ...
      && pokemon.randBattleSeedInt(100, undefined, "Random chance to apply Battle Tag after something makes contact") < this.chance
...
export class PostDefendMoveDisableAbAttr extends PostDefendAbAttr {
  ...
  override canApply({ move, opponent: attacker, pokemon }: PostMoveInteractionAbAttrParams): boolean {
    return (
      ...
      && move.doesFlagEffectApply({ flag: MoveFlags.MAKES_CONTACT, user: attacker, target: pokemon })
      && (this.chance === -1 || pokemon.randBattleSeedInt(100, undefined, "Chance to disable a move") < this.chance)
    );
  }
  ...
  override canApply(params: PostMoveInteractionAbAttrParams): boolean {
    ...
    if (
      ...
      if (heldItems.length > 0) {
        ...
        this.stolenItem = heldItems[pokemon.randBattleSeedInt(heldItems.length, undefined, "Checking if an item can be stolen")];
    ...
    if (
      ...
      && (simulated
        || (!opponent.hasAbilityWithAttr("IgnoreMoveEffectsAbAttr")
          ...
          && pokemon.randBattleSeedInt(100, undefined, "Checking chance to trigger post move effect") < this.chance
          && !pokemon.status))
    ) {
      const effect =
        this.effects.length === 1 ? this.effects[0] : this.effects[pokemon.randBattleSeedInt(this.effects.length, undefined, "Choosing effect to apply")];
...
export class PostAttackApplyBattlerTagAbAttr extends PostAttackAbAttr {
  ...
  override canApply(params: PostMoveInteractionAbAttrParams): boolean {
    ...
    return (
      ...
      && (!this.contactRequired
        || move.doesFlagEffectApply({ flag: MoveFlags.MAKES_CONTACT, user: opponent, target: pokemon }))
      && pokemon.randBattleSeedInt(100, undefined, "Random chance to apply a Battle Tag after attacking") < this.chance(opponent, pokemon, move)
      && !pokemon.status
    );
  }
  ...
  override apply({ pokemon, simulated, opponent }: PostMoveInteractionAbAttrParams): void {
    if (!simulated) {
      const effect =
        this.effects.length === 1 ? this.effects[0] : this.effects[pokemon.randBattleSeedInt(this.effects.length, undefined, "Selecting which tag to apply")];
...
export class PostSummonCopyAbilityAbAttr extends PostSummonAbAttr {
  ...
  override canApply({ pokemon, simulated }: AbAttrBaseParams): boolean {
    ...
    if (targets.length > 1 && !simulated) {
      target = targets[randSeedInt(targets.length, undefined, "Random post summon copy ability")];
    } else {
      target = targets[0];
    }
...
export class PostSummonTransformAbAttr extends PostSummonAbAttr {
  ...
  private getTarget(user: Pokemon): Pokemon | undefined {
    ...
    const mon = targets[user.randBattleSeedInt(targets.length, undefined, "Choose target for post summon ability")];
...
export class ConfusionOnStatusEffectAbAttr extends AbAttr {
  ...
  override apply({ opponent, simulated, pokemon }: ConfusionOnStatusEffectAbAttrParams): void {
    if (!simulated) {
      opponent.addTag(BattlerTagType.CONFUSED, pokemon.randBattleSeedIntRange(2, 5, "Chance to apply effect after attacking"), undefined, opponent.id);
    }
  }
}
...
export class ForewarnAbAttr extends PostSummonAbAttr {
  ...
  override apply({ simulated, pokemon }: AbAttrBaseParams): void {
    ...
    const chosenMove = movesAtMaxPower[pokemon.randBattleSeedInt(movesAtMaxPower.length, undefined, "Choose a forewarn target from equal power moves")];
    globalScene.phaseManager.queueMessage(
...
export class PostTurnRestoreBerryAbAttr extends PostTurnAbAttr {
  ...
  override canApply({ pokemon }: AbAttrBaseParams): boolean {
    ...
    const pass = randSeedFloat("Can apply post turn restore berry chance");
...
export class BypassSpeedChanceAbAttr extends AbAttr {
  ...
  override canApply({ simulated, pokemon }: AbAttrBaseParams): boolean {
    ...
    return (
      !simulated
      && pokemon.randBattleSeedInt(100, undefined, "Decide if speed can be bypassed") < this.chance
      && isDamageMove
...
export function initAbilities() {
  (allAbilities as Ability[]).push(
    ...
    new AbBuilder(AbilityId.HEALER, 5)
      .conditionalAttr(pokemon => pokemon.getAlly() != null && randSeedInt(10, undefined, "Healer chance") < 3, PostTurnResetStatusAbAttr, true)
      .build(),
