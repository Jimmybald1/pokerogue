...
export class PostSummonCopyAbilityAbAttr extends PostSummonAbAttr {
  ...
  override canApply({ pokemon, simulated }: AbAttrBaseParams): boolean {
    ...
    if (targets.length > 1 && !simulated) {
      target = targets[randSeedInt(targets.length, undefined, "Can apply Post Summon Copy Ability")];
    } else {
      target = targets[0];
    }
...
export class PostTurnRestoreBerryAbAttr extends PostTurnAbAttr {
  ...
  override canApply({ pokemon }: AbAttrBaseParams): boolean {
    ...
    const pass = randSeedFloat("Can apply Post Turn Restore Berry");
  ...
  private createEatenBerry(pokemon: Pokemon): boolean {
    ...
    const randomIdx = randSeedInt(this.berriesUnderCap.length, undefined, "Create eaten berry");
...
export class PostBattleLootAbAttr extends PostBattleAbAttr {
  ...
  override canApply({ simulated, victory, pokemon }: PostBattleAbAttrParams): boolean {
    ...
    if (!simulated && postBattleLoot.length > 0 && victory) {
      this.randItem = randSeedItem(postBattleLoot, "Can apply Post Battle Loot");
  ...
  override apply({ pokemon }: PostBattleAbAttrParams): void {
    const postBattleLoot = globalScene.currentBattle.postBattleLoot;
    if (!this.randItem) {
      this.randItem = randSeedItem(postBattleLoot, "Apply Post Battle Loot");
    }
