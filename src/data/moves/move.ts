...
import { BooleanHolder, NumberHolder, isNullOrUndefined, toDmgValue, randSeedItem, randSeedInt, getEnumValues, toReadableString, type Constructor, randSeedFloat, getEnumKeys } from "#app/utils/common";
...
export class StealHeldItemChanceAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const rand = randSeedFloat("Steal held item chance");
    if (rand > this.chance) {
      return false;
    }
...
export class FrenzyAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (!user.getTag(BattlerTagType.FRENZY) && user.getMoveQueue().length === 0) {
      const turnCount = user.randBattleSeedIntRange(1, 2, "Frenzy duration"); // excludes initial use
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const targets = moveTargets.multiple || moveTargets.targets.length === 1
      ...
      : [this.hasTarget
        ...
        : moveTargets.targets[user.randBattleSeedInt(moveTargets.targets.length, undefined, "Randomly selecting a target")]];
