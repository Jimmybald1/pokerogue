...
import { BooleanHolder, NumberHolder, isNullOrUndefined, toDmgValue, randSeedItem, randSeedInt, getEnumValues, toReadableString, type Constructor, getEnumKeys } from "#app/utils/common";
...
export class FixedDamageAttr extends MoveAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    (args[0] as NumberHolder).value = this.getDamage(user, target, move, args);
...
export class RandomLevelDamageAttr extends FixedDamageAttr {
  ...
  getDamage(user: Pokemon, target: Pokemon, move: Move, args: any[]): number {
    if (args[1] == "LOW") {
      return toDmgValue(user.level * 0.5);
    }
    if (args[1] == "HIGH") {
      return toDmgValue(user.level * 1.5);
    }
    return toDmgValue(user.level * (user.randBattleSeedIntRange(50, 150, "Random Damage") * 0.01));
  }
}
...
export class MultiHitAttr extends MoveAttr {
  ...
  getHitCount(user: Pokemon, target: Pokemon): number {
    switch (this.multiHitType) {
      ...
      {
        const rand = user.randBattleSeedInt(20, undefined, "Random number of hits for a 2-to-5-hits move");
...
export class StatusEffectAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const statusCheck = moveChance < 0 || moveChance === 100 || user.randBattleSeedInt(100, undefined, "Chance to apply " + getEnumKeys(StatusEffect)[this.effect]) < moveChance;
...
export class MultiStatusEffectAttr extends StatusEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    this.effect = randSeedItem(this.effects,  "Selecting status effect to apply");
...
export class StealHeldItemChanceAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const stolenItem = tierHeldItems[user.randBattleSeedInt(tierHeldItems.length, undefined, "Selecting an item to steal")];
...
export class RemoveHeldItemAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const removedItem = heldItems[user.randBattleSeedInt(heldItems.length, undefined, "Selecting item to remove")];
...
export class EatBerryAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    this.chosenBerry = heldBerries[user.randBattleSeedInt(heldBerries.length, undefined, "Selecting a berry to eat")];
...
export class StealEatBerryAttr extends EatBerryAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    this.chosenBerry = heldBerries[user.randBattleSeedInt(heldBerries.length, undefined, "Selecting a berry to steal")];
...
export class StatStageChangeAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args?: any[]): boolean {
    ...
    const moveChance = this.getMoveChance(user, target, move, this.selfTarget, true);
    if (moveChance < 0 || moveChance === 100 || user.randBattleSeedInt(100, undefined, "Random chance to raise stat") < moveChance) {
...
export class AcupressureStatStageChangeAttr extends MoveEffectAttr {
  ...
  override apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (randStats.length > 0) {
      const boostStat = [ randStats[user.randBattleSeedInt(randStats.length, undefined, "Choosing stat to raise")] ];
...
const doublePowerChanceMessageFunc = (user: Pokemon, target: Pokemon, move: Move) => {
  ...
  globalScene.executeWithSeedOffset(() => {
    const rand = randSeedInt(100, undefined, "Doubled power chance (applying message)");
    if (rand < move.chance) {
...
export class DoublePowerChanceAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    let rand: number;
    globalScene.executeWithSeedOffset(() => rand = randSeedInt(100, undefined, "Doubled power chance (applying move)"), globalScene.currentBattle.turn << 6, globalScene.waveSeed);
    if (rand! < move.chance) {
      const power = args[0] as NumberHolder;
      power.value *= 2;
      return true;
...
const magnitudeMessageFunc = (user: Pokemon, target: Pokemon, move: Move) => {
  ...
  globalScene.executeWithSeedOffset(() => {
    ...
    const rand = randSeedInt(100, undefined, "Magnitude selection (message)");
...
export class MagnitudePowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    globalScene.executeWithSeedOffset(() => rand = randSeedInt(100, undefined, "Magnitude selection (move)"), globalScene.currentBattle.turn << 6, globalScene.waveSeed);
...
export class PresentPowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const powerSeed = randSeedInt(firstHit ? 100 : 80, undefined, "Present healing chance");
    if (powerSeed <= 40) {
...
export class ShellSideArmCategoryAttr extends VariableMoveCategoryAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (predictedPhysDmg > predictedSpecDmg) {
      category.value = MoveCategory.PHYSICAL;
      return true;
    } else if (predictedPhysDmg === predictedSpecDmg && (args[1] == "SIM" || user.randBattleSeedInt(2, undefined, "Random category for Shell Side Arm") === 0)) {
      category.value = MoveCategory.PHYSICAL;
      return true;
    }
    return false;
...
export class FrenzyAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (!user.getTag(BattlerTagType.FRENZY) && !user.getMoveQueue().length) {
      const turnCount = user.randBattleSeedIntRange(1, 2, "Frenzy duration");
...
export class AddBattlerTagAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const moveChance = this.getMoveChance(user, target, move, this.selfTarget, true);
    if (moveChance < 0 || moveChance === 100 || user.randBattleSeedInt(100, undefined, "Chance to add Battler Tag") < moveChance) {
      return (this.selfTarget ? user : target).addTag(this.tagType,  user.randBattleSeedIntRange(this.turnCountMin, this.turnCountMax, "Battler Tag duration"), move.id, user.id);
    }
...
export class JawLockAttr extends AddBattlerTagAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (moveChance < 0 || moveChance === 100 || user.randBattleSeedInt(100, undefined, "Chance to apply Trap tag (Jaw Lock)") < moveChance) {
      /**
...
export class ProtectAttr extends AddBattlerTagAttr {
  ...
  getCondition(): MoveConditionFunc {
    return ((user, target, move): boolean => {
      ...
      if (timesUsed) {
        return !user.randBattleSeedInt(Math.pow(3, timesUsed), undefined, "Chance for Protection move to succeed");
      }
      return true;
    });
  }
...
export class AddArenaTagAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if ((move.chance < 0 || move.chance === 100 || user.randBattleSeedInt(100, undefined, "Chance to add arena tag") < move.chance) && user.getLastXMoves(1)[0]?.result === MoveResult.SUCCESS) {
...
export class AddArenaTrapTagHitAttr extends AddArenaTagAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const side = (this.selfSideTarget ? user : target).isPlayer() ? ArenaTagSide.PLAYER : ArenaTagSide.ENEMY;
    const tag = globalScene.arena.getTagOnSide(this.tagType, side) as ArenaTrapTag;
    if ((moveChance < 0 || moveChance === 100 || user.randBattleSeedInt(100, undefined, "Chance to add arena tag on hit") < moveChance) && user.getLastXMoves(1)[0]?.result === MoveResult.SUCCESS) {
...
export class RevivalBlessingAttr extends MoveEffectAttr {
  ...
  override apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (user instanceof PlayerPokemon) {
      ...
      const pokemon = faintedPokemon[user.randBattleSeedInt(faintedPokemon.length, undefined, "Choosing Pokemon to revive")];
...
class CallMoveAttr extends OverrideMoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const targets = moveTargets.multiple || moveTargets.targets.length === 1
      ? moveTargets.targets
      : [ this.hasTarget ? target.getBattlerIndex() : moveTargets.targets[user.randBattleSeedInt(moveTargets.targets.length, undefined, "Randomly selecting a target")] ]; // account for Mirror Move having a target already
...
export class RandomMoveAttr extends CallMoveAttr {
  ...
  override apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    do {
      moveId = this.getMoveOverride() ?? moveIds[user.randBattleSeedInt(moveIds.length, undefined, "Randomly selecting any valid move")];
    }
...
export class RandomMovesetMoveAttr extends CallMoveAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) => {
      ...
      this.moveId = moves[user.randBattleSeedInt(moves.length, undefined, "Randomly selecting a valid move")]!.moveId;
      return true;
    };
  }
}
...
export class ResistLastMoveTypeAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const type = validTypes[user.randBattleSeedInt(validTypes.length, undefined, "Choosing type to transform into (Conversion2)")];
...
export function getMoveTargets(user: Pokemon, move: Moves, replaceTarget?: MoveTarget): MoveTargetSet {
  ...
  switch (moveTarget) {
    ...
    case MoveTarget.RANDOM_NEAR_ENEMY:
      set = [ opponents[user.randBattleSeedInt(opponents.length, undefined, "Randomly selecting an opponent to attack")] ];
      break;
