...
export class FixedDamageAttr extends MoveAttr {
  ...
  getDamage(_user: Pokemon, _target: Pokemon, _move: Move, args: any[]): number {
    return this.damage;
  }
}
...
export class LevelDamageAttr extends FixedDamageAttr {
  ...
  getDamage(user: Pokemon, _target: Pokemon, _move: Move, args?: any[]): number {
    return user.level;
  }
}
...
export class DoublePowerChanceAttr extends VariablePowerAttr {
  ...
  apply(_user: Pokemon, _target: Pokemon, _move: Move, args: any[]): boolean {
    let rand = 0;
    globalScene.executeWithSeedOffset(
      () => (rand = randSeedInt(100, undefined, "Doubled power chance (applying move)")),
      globalScene.currentBattle.turn << 6,
      globalScene.waveSeed,
    );
    if (rand < this.chance) {
...
export class MagnitudePowerAttr extends VariablePowerAttr {
  apply(_user: Pokemon, _target: Pokemon, _move: Move, args: [NumberHolder, ...any[]]): boolean {
    ...
    globalScene.executeWithSeedOffset(
      () => {
        const rand = randSeedInt(100, undefined, "Magnitude selection (move)");
...
export class ShellSideArmCategoryAttr extends VariableMoveCategoryAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (predictedPhysDmg === predictedSpecDmg && (args[1] == "SIM" || user.randBattleSeedInt(2, undefined, "Random category for Shell Side Arm") === 0)) {
      category.value = MoveCategory.PHYSICAL;
      return true;
    }
    return false;
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (moveChance < 0 || moveChance === 100 || user.randBattleSeedInt(100, undefined, "Chance to add Battler Tag") < moveChance) {
      return (this.selfTarget ? user : target).addTag(
        this.tagType,
        user.randBattleSeedIntRange(this.turnCountMin, this.turnCountMax, "Battler Tag duration"),
        move.id,
        user.id,
      );
    }
...
export class AddArenaTagAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      (move.chance < 0 || move.chance === 100 || user.randBattleSeedInt(100, undefined, "Chance to add arena tag") < move.chance)
...
export class AddArenaTrapTagHitAttr extends AddArenaTagAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, _args: any[]): boolean {
    ...
    const tag = globalScene.arena.getTagOnSide(this.tagType, side) as EntryHazardTag;
    if (
      (moveChance < 0 || moveChance === 100 || user.randBattleSeedInt(100, undefined, "Chance to add arena tag on hit") < moveChance)
      && user.getLastXMoves(1)[0]?.result === MoveResult.SUCCESS
    ) {
