...
type AbAttrSuccessFunc<TAttr extends AbAttr> = (attr: TAttr, passive: boolean, simulated: boolean) => boolean;
...
export class PostDefendContactApplyTagChanceAbAttr extends PostDefendAbAttr {
  ...
  override canApplyPostDefend(pokemon: Pokemon, passive: boolean, simulated: boolean, attacker: Pokemon, move: Move, hitResult: HitResult | null, args: any[]): boolean {
    return move.checkFlag(MoveFlags.MAKES_CONTACT, attacker, pokemon) && pokemon.randSeedInt(100, undefined, "Random chance to apply Battle Tag after something makes contact") < this.chance
...
export class PostDefendMoveDisableAbAttr extends PostDefendAbAttr {
  ...
  override canApplyPostDefend(pokemon: Pokemon, passive: boolean, simulated: boolean, attacker: Pokemon, move: Move, hitResult: HitResult | null, args: any[]): boolean {
    return attacker.getTag(BattlerTagType.DISABLED) === null && !move.hitsSubstitute(attacker, pokemon)
      && move.checkFlag(MoveFlags.MAKES_CONTACT, attacker, pokemon) && (this.chance === -1 || pokemon.randSeedInt(100, undefined, "Chance to disable a move") < this.chance);
  }
...
export class PostBattleLootAbAttr extends PostBattleAbAttr {
  ...
  override applyPostBattle(pokemon: Pokemon, passive: boolean, simulated: boolean, args: any[]): void {
    const postBattleLoot = globalScene.currentBattle.postBattleLoot;
    if (!this.randItem) {
      this.randItem = Utils.randSeedItem(postBattleLoot, "Randomly selecting item to Pickup");
    }
...
function applySingleAbAttrs<TAttr extends AbAttr>(
  ...
  for (const attr of ability.getAttrs(attrType)) {
    ...
    let abShown = false;
    if (condition && !condition(pokemon) || !successFunc(attr, passive, false)) {
      continue;
    }
...
export function initAbilities() {
  allAbilities.push(
    ...
    new Ability(Abilities.HEALER, 5)
      .conditionalAttr(pokemon => !Utils.isNullOrUndefined(pokemon.getAlly()) && Utils.randSeedInt(10, undefined, "Random chance to apply Healer") < 3, PostTurnResetStatusAbAttr, true),
