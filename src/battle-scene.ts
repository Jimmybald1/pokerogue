...
export class BattleScene extends SceneBase {
  ...
  addEnemyPokemon(
    ...
    if (boss && !dataSource) {
      const secondaryIvs = getIvsFromId(randSeedInt(4294967296, undefined, "IVs"));
  ...
  private generateNewBattleTrainer(waveIndex: number): Trainer {
    ...
    if (config.doubleOnly) {
      ...
      doubleTrainer = randSeedInt(this.getDoubleBattleChance(waveIndex), undefined, "Double battle trainer chance") === 0;
    }

    const overrideVariant = doubleTrainer
      ? TrainerVariant.DOUBLE
      : activeOverrides.RANDOM_TRAINER_OVERRIDE?.trainerVariant;
    const variant = overrideVariant ?? (randSeedInt(2, undefined, "Trainer gender") ? TrainerVariant.FEMALE : TrainerVariant.DEFAULT);
  ...
  private checkIsDouble({ double: forcedDouble, battleType, waveIndex, trainer }: NewBattleConstructedProps): boolean {
    ...
    if (battleType === BattleType.WILD) {
      return randSeedInt(this.getDoubleBattleChance(waveIndex), undefined, "Check if was wild double") === 0;
    }
  ...
  public getSpeciesFormIndex(species: PokemonSpecies, gender?: Gender, nature?: Nature, ignoreArena = false): number {
    ...
    switch (species.speciesId) {
      ...
      case SpeciesId.PALDEA_TAUROS:
        return randSeedInt(species.forms.length, undefined, "Species Form Default");
      ...
      case SpeciesId.SINISTCHA:
        return randSeedInt(16, undefined, "Polteageist, Sinistcha, Maushold and Dudunsparce Forms") ? 0 : 1;
      case SpeciesId.PICHU:
        return randSeedInt(8, undefined, "Pichu Form") ? 0 : 1;
      case SpeciesId.PIKACHU:
        if (isTrainerBattle && this.currentBattle?.waveIndex < 30) {
          return 0; // Ban Cosplay and Partner Pika from Trainers before wave 30
        }
        return randSeedInt(8, 0, "Species Form Pikachu");
      case SpeciesId.EEVEE:
        if (isTrainerBattle && this.currentBattle?.waveIndex < 30 && !isEggPhase) {
          return 0; // No Partner Eevee for Wave 12 Preschoolers
        }
        return randSeedInt(2, undefined, "Species Form Eevee");
      case SpeciesId.MAGEARNA:
      case SpeciesId.URSHIFU:
        return randSeedInt(2, undefined, "Species Form Urshifu");
      case SpeciesId.TATSUGIRI:
        return randSeedInt(3, undefined, "Species Form Tatsugiri");
      case SpeciesId.ZYGARDE:
        return randSeedInt(4, undefined, "Species Form Zygarde");
      case SpeciesId.MINIOR:
        return randSeedInt(7, undefined, "Species Form Minior");
      case SpeciesId.ALCREMIE:
        return randSeedInt(9, undefined, "Species Form Alcremie");
      ...
      case SpeciesId.GIMMIGHOUL:
        ...
        return randSeedInt(species.forms.length, 0, "Species Form Gimmighoul");
      case SpeciesId.BURMY:
      case SpeciesId.WORMADAM:
        if (ignoreArena) {
          return randSeedInt(species.forms.length, undefined, "Wormadam form selection");
        }
      ...
      case SpeciesId.LYCANROC:
        if (ignoreArena) {
          return randSeedInt(species.forms.length, undefined, "Lycanroc form selection");
        }
  ...
  private getGeneratedOffsetGym(): boolean {
    ...
    this.executeWithSeedOffset(
      () => {
        ret = !randSeedInt(2, undefined, "Random gym offset");
      },
      0,
      this.seed.toString(),
    );
  ...
  private getGeneratedWaveCycleOffset(): number {
    ...
    this.executeWithSeedOffset(
      () => {
        ret = randSeedInt(8, undefined, "Random day/night cycle offset  5 x") * 5;
      },
      0,
      this.seed.toString(),
    );
      ...
      this.executeWithSeedOffset(() => {
        isBoss =
          ...
          || (this.gameMode.hasRandomBosses
            && randSeedInt(100, undefined, "Boss HP segments") < Math.min(Math.max(Math.ceil((waveIndex - 250) / 50), 0) * 2, 30));
      }, waveIndex << 2);
    }
    if (!isBoss) {
      return 0;
  ...
  trySpreadPokerus(): void {
    ...
    const spread = (index: number, spreadTo: number) => {
      ...
      if (!partyMember.pokerus && !randSeedInt(10, undefined, "Pokerus spread chance")) {
  ...
  randomSpecies(
    ...
    return randSeedItem(filteredSpecies, "Random Species");
  }
  ...
  generateRandomBiome(waveIndex: number): BiomeId {
    ...
    const randInt = randSeedInt(totalWeight, undefined, "Random biome");
    ...
    return randSeedItem(biomes, "Fully Random Biome");
  }
      ...
      party.forEach((enemyPokemon: EnemyPokemon, i: number) => {
        if (heldModifiersConfigs && i < heldModifiersConfigs.length && heldModifiersConfigs[i]) {
          ...
          for (let c = 0; c < chances; c++) {
            if (!randSeedInt(this.gameMode.getEnemyModifierChance(isBoss), undefined, "Modifier roll")) {
              count++;
            }
          }
          if (isBoss) {
    ...
    this.executeWithSeedOffset(
      () => {
        const shuffleModifiers = mods => {
          ...
          const rand = randSeedInt(mods.length, undefined, "Apply shuffled modifiers");
