    ...
    return new Promise(async (resolve, reject) => {
          ...
          for (let i = 0; i <= 5; i++) {
            if (sessionData.party[i]) {
              ...
              if (sessionData.party[i]?.abilityIndex === 1) {
                if (speciesToCheck.ability1 === speciesToCheck.ability2 && speciesToCheck.abilityHidden !== AbilityId.NONE && speciesToCheck.abilityHidden !== speciesToCheck.ability1) {
      ...
      if (!bypassLogin && !localStorage.getItem(`sessionData${slotId ? slotId : ""}_${loggedInUser?.username}`)) {
        pokerogueApi.savedata.session.get({ slot: slotId, clientSessionId }).then(async response => {
          ...
          localStorage.setItem(
            `sessionData${slotId ? slotId : ""}_${loggedInUser ? loggedInUser.username : "Guest"}`,
