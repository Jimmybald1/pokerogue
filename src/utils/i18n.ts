 ...
 */
export function getRandomLocaleEntry(key: string, reason?: string): [key: string, value: string] {
  const keyName = `${key}.${randSeedItem(Object.keys(i18next.t(key, { returnObjects: true })), reason)}`;
